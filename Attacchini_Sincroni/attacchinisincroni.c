...
{ 
	...
	while(1) {
		...
		DBGpthread_mutex_lock(&mutex, "Prendo Mutex come Attacchino");
		...
		*/
		...
		if (numVecchiettiInLettura>=0) {
			DBGpthread_cond_wait(&condAttacchini, &mutex, "Aspetto Vecch Finiscono");
		}
		...
		/*  COMPLETARE */
		if (numAttacchiniInAttesa==2) {
			DBGpthread_cond_signal(&condAttesaArrivoAttacchino2,"Aspetto Secondo Attach");
			numAttacchiniAlLavoro=2;
		} 
		if (numAttacchiniInAttesa==1){
			DBGpthread_cond_wait(&condAttesaArrivoAttacchino2, &mutex, "Aspetto Secondo Attach");
			numAttacchiniInAttesa=0;
			numAttacchiniAlLavoro=1;
		}
		...
		*/
		...
		/*  COMPLETARE */
		if (numAttacchiniAlLavoro==2) {
			printf("Aspetto altro Attach");
			numAttacchiniAlLavoro=1;
			DBGpthread_cond_wait(&condAttesaFineAttacchino2, &mutex, "Aspetto Secondo Attacchino");
		}
		numAttacchiniAlLavoro=0;
		DBGpthread_cond_signal(&condAttesaFineAttacchino2, "Finito Tutti attacchini");
		DBGpthread_cond_broadcast(&condVecchietti, "Sveglio i Vecch");
