...
{ 
	while(1) {
		...
		if ( NumSoccorritoriArrivati!=2 ) {
			DBGpthread_cond_wait(&condInizioSoccorso, &mutex, "Aspetto Altri Soccorsi" );
		}
		...
		DBGpthread_cond_signal(&condFineSoccorso, "Fine Soccorso");
...
{ 
	...
	if ( rc ) PrintERROR_andExit(rc, "Cond Init Failed");
